...
//						$scope.skuList = data.result.skuList;
//					}, function(err) {
//
//					});
					
					$scope.itemDetail = $stateParams.itemDetail;
					$scope.skuList = $stateParams.itemDetail.skuList;
					...
					$scope.selectMall = function($event, ele, type) {
						...
						if (type == 'attachment') {
							var attSelected = [];
							// 遍历配套,改变selected字段值
							for (k in $scope.itemDetail.attachment.list) {
								for (i in $scope.itemDetail.attachment.list[k].skuPropertyList) {
									var id = $scope.itemDetail.attachment.list[k].skuPropertyList[i].id;
									if (ele.pid == id) {
										for (j in $scope.itemDetail.attachment.list[k].skuPropertyList[i].eleList) {
											var aId = $scope.itemDetail.attachment.list[k].skuPropertyList[i].eleList[j].id;
											if (aId == ele.id) {
												$scope.itemDetail.attachment.list[k].skuPropertyList[i].eleList[j].isSelected = 1;
											} else {
												$scope.itemDetail.attachment.list[k].skuPropertyList[i].eleList[j].isSelected = 0;
											}
										}
										break;
									}
							...
							for (k in $scope.itemDetail.attachment.list) {
								for (i in $scope.itemDetail.attachment.list[k].skuPropertyList) {
									for (j in $scope.itemDetail.attachment.list[k].skuPropertyList[i].eleList) {
										var temEle = $scope.itemDetail.attachment.list[k].skuPropertyList[i].eleList[j];
							...
							if (attSelected.length > 0) {
								if (attSelected.length > 1) {
									...
									attSelected = attSelected.splice(1,
											attSelected.length - 1);
									for (i in tempList) {
									...
									if (attResultList.length > 0) {
										for (i in attResultList) {
											...
											for (k in $scope.itemDetail.attachment.list) {
												for (j in $scope.itemDetail.attachment.list[k].skuList) {
													var tempSkuId = $scope.itemDetail.attachment.list[k].skuList[j].skuId;
													var price = $scope.itemDetail.attachment.list[k].skuList[j].price;
													if (skuId == tempSkuId) {
														if (attPrice.length > 0) {
															totalPrice = totalPrice
																	- attPrice[attPrice.length - 1];
															attPrice = attPrice
																	.splice(
																			attPrice.length - 2,
																			attPrice.length - 1);
										...
										for (k in $scope.itemDetail.attachment.list) {
											for (j in $scope.itemDetail.attachment.list[k].skuList) {
												...
												if (skuId == tempSkuId) {
													...
													$(
															"#att"
																	+ $scope.itemDetail.attachment.list[k].goodsId)
															.html(
																	'￥'
																			+ attPrice[attPrice.length - 1]);
													break;
										...
										$("#mallPrice")
												.html(
														"￥"
																...
																+ "～"
																+ priceArr
																		.sort(compare)[priceArr.length - 1]);
									} else if (priceArr.length == 1) {
										if (attPrice.length > 0) {
											totalPrice = totalPrice
													- attPrice[attPrice.length - 1];
											attPrice = attPrice.splice(
													attPrice.length - 2,
													attPrice.length - 1);
										}
							...
							var eleSelected = [];
							// 遍历主商品,改变selected字段值
							for (i in $scope.itemDetail.skuPropertyList) {
								var id = $scope.itemDetail.skuPropertyList[i].id;
								if (ele.pid == id) {
									for (j in $scope.itemDetail.skuPropertyList[i].eleList) {
										var sId = $scope.itemDetail.skuPropertyList[i].eleList[j].id;
										if (sId == ele.id) {
											$scope.itemDetail.skuPropertyList[i].eleList[j].isSelected = 1;
										} else {
											$scope.itemDetail.skuPropertyList[i].eleList[j].isSelected = 0;
										}
									}
									break;
								}
							...
							for (i in $scope.itemDetail.skuPropertyList) {
								for (j in $scope.itemDetail.skuPropertyList[i].eleList) {
									var tempEle = $scope.itemDetail.skuPropertyList[i].eleList[j];
							...
							if (eleSelected.length > 0) {
								if (eleSelected.length > 1) {
									...
									eleSelected = eleSelected.splice(1,
											eleSelected.length - 1);
									for (i in tempList) {
										var skuId = tempList[i];
